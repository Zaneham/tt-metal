...
void issue_restore_reads(
    ...
    // Issue max reads
    CircularBuffer cb_max(cb_max_in);
    cb_max.reserve_back(Sq_chunk_t);
    uint32_t max_tile_id = stats_tile_logical.id_of(nb, nq, stats_seq_start_tile, 0);
    uint32_t max_addr = cb_max.get_write_ptr();
    for (uint32_t r = 0; r < stats_rows; ++r) {
        noc.async_read(stats_writer, CoreLocalMem<uint32_t>(max_addr), stats_tile_bytes, {.page_id = max_tile_id}, {});
        max_tile_id += stats_row_stride;
        max_addr += stats_tile_bytes;
    }

    // Issue sum reads
    CircularBuffer cb_sum(cb_sum_in);
    cb_sum.reserve_back(Sq_chunk_t);
    uint32_t sum_tile_id = stats_tile_logical.id_of(nb, nq, sum_offset + stats_seq_start_tile, 0);
    uint32_t sum_addr = cb_sum.get_write_ptr();
    for (uint32_t r = 0; r < stats_rows; ++r) {
        noc.async_read(stats_writer, CoreLocalMem<uint32_t>(sum_addr), stats_tile_bytes, {.page_id = sum_tile_id}, {});
...
void save_accumulators_with_trid(
    ...
    write_block_row_grouped_trid(noc, cat_out_generator, out_slice, end_seq_tile, cb_out, tile_bytes, sbh, save_trid);
    ...
    uint32_t max_write_addr = cb_max.get_read_ptr();
    for (uint32_t i = stats_seq_start_tile; i < stats_seq_end_tile; i++) {
        noc.async_write(
            CoreLocalMem<uint32_t>(max_write_addr),
            stats_writer,
            stats_tile_bytes,
            {},
            {.page_id = stats_tile_logical.id_of(nb, nq, i, 0)});
    ...
    uint32_t sum_write_addr = cb_sum.get_read_ptr();
    for (uint32_t i = stats_seq_start_tile; i < stats_seq_end_tile; i++) {
        noc.async_write(
            CoreLocalMem<uint32_t>(sum_write_addr),
            stats_writer,
            stats_tile_bytes,
            {},
            {.page_id = stats_tile_logical.id_of(nb, nq, sum_offset + i, 0)});
...
void kernel_main() {
    ...
    if constexpr (needs_lightweight_mask) {
        generate_lightweight_mask_tiles<global_n_partial_col, joint_l_partial_col, cb_mask_in, is_causal>(noc);
    }
