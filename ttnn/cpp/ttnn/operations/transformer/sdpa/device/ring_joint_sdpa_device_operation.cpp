...
void RingJointSDPADeviceOperation::validate_on_program_cache_miss(
    ...
    const uint32_t VDH = tensor_args.v_head_dim();
    ...
    TT_FATAL(
        ...
        L);
    if (has_latent_v) {
        TT_FATAL(NVH == 1, "Latent-V mode currently requires one shared V head. Got V heads: {}", NVH);
        TT_FATAL(
            VDH <= DH,
...
RingJointSDPAResultSpec RingJointSDPADeviceOperation::compute_output_specs(
    ...
    out_shape[3] = tensor_args.v_head_dim();
    auto joint_out_shape = joint_input.logical_shape();
    joint_out_shape[3] = tensor_args.v_head_dim();
...
tt::tt_metal::operation::OpPerformanceModelGeneral<Tensors> RingJointSDPADeviceOperation::create_op_performance_model(
    ...
    const uint32_t DV = tensor_args.v_head_dim();
...
RingJointSDPAResult ring_joint_scaled_dot_product_attention(
    ...
    auto tensor_args = OperationType::tensor_args_t{
        ...
        .latent_v_head_dim = head_dim_v.value_or(0)};
