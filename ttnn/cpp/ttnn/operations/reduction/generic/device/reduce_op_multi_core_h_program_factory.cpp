...
tt::tt_metal::ProgramDescriptor ReduceDeviceOperation::ReduceMultiCoreHProgramFactory::create_descriptor(
    ...
    if (rm_path) {
        std::vector<uint32_t> reader_compile_time_args =
            build_rm_reader_ct_args(plan, scaler_bits, *src0_buffer, ReduceOpDim::H);
    ...
    if (rm_path) {
        std::vector<uint32_t> writer_compile_time_args = build_rm_writer_ct_args(plan, *dst_buffer, ReduceOpDim::H);
