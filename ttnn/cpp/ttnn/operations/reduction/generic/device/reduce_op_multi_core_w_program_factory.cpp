...
tt::tt_metal::ProgramDescriptor ReduceDeviceOperation::ReduceMultiCoreWProgramFactory::create_descriptor(
    ...
    if (rm_path) {
        reader_compile_time_args = build_rm_reader_ct_args(
            plan, std::bit_cast<uint32_t>(operation_attributes.scaler), *src_buffer, ReduceOpDim::W);
    } else {
    ...
    if (rm_path) {
        writer_compile_time_args = build_rm_writer_ct_args(plan, *dst_buffer, ReduceOpDim::W);
    } else {
