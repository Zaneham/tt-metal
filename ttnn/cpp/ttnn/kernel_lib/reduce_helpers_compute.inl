                    ...
                    if constexpr (is_sfpu) {
                        ...
                        if constexpr (waits_per_tile(input_policy)) {
                            ...
                            detail::sfpu_copy_and_fold_max<reduce_format>(
                                input_cb_id, 0, dst_idx, sfpu_work_dst, wt == 0);
                            input_cb.pop_front(onetile);
                        } else if constexpr (waits_bulk(input_policy)) {
                            detail::sfpu_copy_and_fold_max<reduce_format>(
                                input_cb_id, wt, dst_idx, sfpu_work_dst, wt == 0);
                        } else {
                            detail::sfpu_copy_and_fold_max<reduce_format>(
                                input_cb_id, wt + index_offset, dst_idx, sfpu_work_dst, wt == 0);
                        }
                    ...
                    for (uint32_t i = wt; i < chunk_end; ++i) {
                        if constexpr (is_sfpu) {
                            ...
                            if constexpr (waits_per_tile(input_policy)) {
                                input_cb.wait_front(onetile);
                                detail::sfpu_copy_and_fold_max<reduce_format>(
                                    input_cb_id, 0, dst_idx, sfpu_work_dst, ht == 0);
                                input_cb.pop_front(onetile);
                            } else if constexpr (waits_bulk(input_policy)) {
                                const uint32_t tile_idx = ht * current_chunk + (i - wt);
                                detail::sfpu_copy_and_fold_max<reduce_format>(
                                    input_cb_id, tile_idx, dst_idx, sfpu_work_dst, ht == 0);
                            } else {
