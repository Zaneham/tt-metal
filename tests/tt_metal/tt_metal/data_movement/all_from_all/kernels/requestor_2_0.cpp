...
void kernel_main() {
    ...
    // varargs: [0]=num_of_transactions, [1]=bytes_per_transaction,
    // [2..2+2*num_subordinates-1]=subordinate coord pairs (x,y).
    const uint32_t num_of_transactions = get_vararg(0);
    const uint32_t bytes_per_transaction_per_subordinate = get_vararg(1);
