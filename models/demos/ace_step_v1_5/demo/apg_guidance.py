"""
Local copy of APG / ADG guidance math from ACE-Step.
...
def _cos_similarity(tensor1: torch.Tensor, tensor2: torch.Tensor) -> torch.Tensor:
...
def _perpendicular_component(
    latent_diff: torch.Tensor, latent_hat_uncond: torch.Tensor
    ...
    perpendicular = latent_diff - projection

    return projection.view(n, t, c), perpendicular.reshape(n, t, c)
...
def adg_forward(
    ...
    if latents.shape[1] != noise_pred_cond.shape[1]:
        if noise_pred_cond.shape[1] % latents.shape[1] != 0:
            raise ValueError("noise_pred_cond time dim must be a multiple of latents time dim.")
    ...
    proj, perp = _perpendicular_component(latent_diff, latent_hat_uncond)
