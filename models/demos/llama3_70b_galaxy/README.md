...
```
MESH_DEVICE=TG TT_LLAMA_TEXT_VER=llama3_70b_galaxy VLLM_RPC_TIMEOUT=900000 python plugins/vllm-tt-plugin/examples/server_example_tt.py --model "meta-llama/Llama-3.3-70B-Instruct" --max_num_seqs 8 --async-scheduling --plugin-config '{"tt": {"tt_data_parallel_size": 4, "dispatch_core_axis": "col", "sample_on_device_mode": "all", "fabric_config": "FABRIC_1D_RING", "worker_l1_size": 1344544, "trace_region_size": 220000000}}'
```

`max_num_seqs` remains per lane in this configuration, so `tt_data_parallel_size=4` and `max_num_seqs=8` gives the same 32-request global concurrency as the older DP=4 setup.
