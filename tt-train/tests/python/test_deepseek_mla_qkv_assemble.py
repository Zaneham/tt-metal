...
"""Focused parity coverage for DeepSeek MLA QKV assemble integration."""
...
def _old_reference_forward(module: MultiHeadLatentAttention, x: ttml.autograd.Tensor, mask: ttml.autograd.Tensor):
...
def test_mla_qkv_assemble_matches_reference_forward_and_backward():
    ...
    try:
        ...
        x, mask = _make_inputs()
        expected = _old_reference_forward(module, x, mask)
        ...
        ctx.reset_graph()
        _assert_close("forward", _compare_arrays(expected_np, actual_np))
        ...
        zero_grad.zero_grad()
        x, mask = _make_inputs()
        loss = ttml.ops.unary.mean(_old_reference_forward(module, x, mask))
        loss.backward(False)
        ttnn.synchronize_device(device)
        expected_grads = {
        ...
        assert set(expected_grads) == set(actual_grads)
        for name in sorted(expected_grads):
            _assert_close(f"grad {name}", _compare_arrays(expected_grads[name], actual_grads[name]))
    finally:
        ctx.close_device()
